...
class CuttingOptimizer:
    ...
    def _transform_item_for_calculation(self, item: ItemInput,panel:PanelInput) -> Dict[str, Any]:
        ...
        if item.grain != panel.grain:
            ...
            """ engrand_line = 'vertical' if item.engraved_line == 'horizontal' else 'horizontal'
            if item.engraved_line =='none': engrand_line='none' """
        else:
            # For vertical grain, keep original dimensions
            cutting_width = item.width
            cutting_height = item.height
            """  engrand_line = item.engraved_line """
    ...
    def calculate(self, user_params: UserParams, method: str) -> CalculationResponse:
        """Perform the cutting optimization calculation."""
        import time
        ...
        try:
            # Expand inputs
            panels_list = self._expand_panels(user_params.panels)
            items_list = self._expand_items(user_params.items,user_params.panels[0])

            logger.info(f"Expanded to {len(panels_list)} panels and {len(items_list)} items")

            # Prepare optimization parameters
            opcut_params = common.Params(
                cut_width=user_params.cut_width,
                min_initial_usage=user_params.min_initial_usage,
                panels=panels_list,
                items=items_list
            )
            ...
            # Transform result
            transformed_result = self.transform_result(result_json, user_params)
